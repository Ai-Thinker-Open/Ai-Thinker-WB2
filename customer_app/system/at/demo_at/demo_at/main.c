...
static int cmd_atcmd_response(uint8_t *data, int len)
{
    int i;
    for (i = 0; i < len; i++)
        bl_uart_data_send(0, data[i]);
    return len;
}

void cmd_atcmd(char *buf, int len, int argc, char **argv)
{
    if (argc != 2) {
        printf("AT cmd parameter number is error!\r\n");
        return;
    }
    ...
    if (cmd_buf) {
        ...
        at_module_func(cmd_buf, cmd_atcmd_response);
        free(cmd_buf);
    }
}
