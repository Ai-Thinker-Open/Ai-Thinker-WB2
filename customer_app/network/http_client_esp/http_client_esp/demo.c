...
const void *postman_root_cert_pem_start = "-----BEGIN CERTIFICATE-----\n<FIXME>\n-----END CERTIFICATE-----";
...
#define MD(fun) { fun, #fun }
static const struct {
    void (*fun)(void);
    const char *desc;
} http_demos[] = {
    MD(http_rest_with_url),
    MD(http_rest_with_hostname_path),
#if CONFIG_ESP_HTTP_CLIENT_ENABLE_BASIC_AUTH
    MD(http_auth_basic),
    MD(http_auth_basic_redirect),
#endif
#if CONFIG_ESP_HTTP_CLIENT_ENABLE_DIGEST_AUTH
    MD(http_auth_digest),
#endif
    MD(http_relative_redirect),
    MD(http_absolute_redirect),
    MD(http_absolute_redirect_manual),
#if CONFIG_MBEDTLS_CERTIFICATE_BUNDLE
    MD(https_with_url),
#endif
    MD(https_with_hostname_path),
    MD(http_redirect_to_https),
    MD(http_download_chunk),
    MD(http_perform_as_stream_reader),
    MD(https_async),
    MD(https_with_invalid_url),
    MD(http_native_request),
#if CONFIG_MBEDTLS_CERTIFICATE_BUNDLE
    MD(http_partial_download),
#endif
};

static void http_test_task(void *pvParameters)
{
    for (int i = 0; i < sizeof(http_demos) / sizeof(http_demos[0]); ++i) {
        ESP_LOGD(TAG, "====================== Running %s ====================", http_demos[i].desc);
        http_demos[i].fun();
    }
...
static void cmd_http(char *buf, int len, int argc, char **argv)
{
...
const static struct cli_command cmds_user[] STATIC_CLI_CMD_ATTRIBUTE = {
    {"http", "demo http client", cmd_http},
};
