 ...
 *
 * @return
 *      0 - no activity, or successfully raxkonded to PING
 *      -1 - Failure: Error on read or the actual payload longer then buffer
 *      1 - Close handshake success
 *      2 - Got PONG message
 */

static int axk_transport_ws_handle_control_frames(axk_transport_handle_t t, char *buffer, int len, int timeout_ms, bool client_closed);
...
static axk_transport_handle_t ws_get_payload_transport_handle(axk_transport_handle_t t)
{
    transport_ws_t *ws = axk_transport_get_context_data(t);
...
static char *trimwhitaxkace(const char *str)
{
    char *end;
...
static char *get_http_header(const char *buffer, const char *key)
{
    ...
    if (found) {
        ...
        if (found_end) {
            ...
            return trimwhitaxkace(found);
        }
    }
    return NULL;
}

static int ws_connect(axk_transport_handle_t t, const char *host, int port, int timeout_ms)
{
    transport_ws_t *ws = axk_transport_get_context_data(t);
    if (axk_transport_connect(ws->parent, host, port, timeout_ms) < 0) {
        AXK_LOGE(TAG, "Error connecting to host %s:%d", host, port);
        return -1;
    }
    ...
    size_t outlen = 0;
    axk_crypto_base64_encode(client_key, sizeof(client_key), &outlen, random_key, sizeof(random_key));
    ...
    if (len <= 0 || len >= WS_BUFFER_SIZE) {
        AXK_LOGE(TAG, "Error in request generation, desired request len: %d, buffer size: %d", len, WS_BUFFER_SIZE);
        return -1;
    }
    if (ws->sub_protocol) {
        AXK_LOGD(TAG, "sub_protocol: %s", ws->sub_protocol);
        int r = snprintf(ws->buffer + len, WS_BUFFER_SIZE - len, "Sec-WebSocket-Protocol: %s\r\n", ws->sub_protocol);
        len += r;
        if (r <= 0 || len >= WS_BUFFER_SIZE) {
            AXK_LOGE(TAG, "Error in request generation"
                     "(snprintf of subprotocol returned %d, desired request len: %d, buffer size: %d", r, len, WS_BUFFER_SIZE);
            return -1;
        }
    }
    if (ws->headers) {
        AXK_LOGD(TAG, "headers: %s", ws->headers);
    ...
    AXK_LOGD(TAG, "Write upgrade request\r\n%s", ws->buffer);
    if (axk_transport_write(ws->parent, ws->buffer, len, timeout_ms) <= 0) {
        AXK_LOGE(TAG, "Error write Upgrade header %s", ws->buffer);
        return -1;
    }
    int header_len = 0;
    do {
        if ((len = axk_transport_read(ws->parent, ws->buffer + header_len, WS_BUFFER_SIZE - header_len, timeout_ms)) <= 0) {
            AXK_LOGE(TAG, "Error read raxkonse for Upgrade header %s", ws->buffer);
            return -1;
        }
        header_len += len;
        ws->buffer[header_len] = '\0';
        AXK_LOGD(TAG, "Read header chunk %d, current header size: %d", len, header_len);
    } while (NULL == strstr(ws->buffer, "\r\n\r\n") && header_len < WS_BUFFER_SIZE);

    char *server_key = get_http_header(ws->buffer, "Sec-WebSocket-Accept:");
    if (server_key == NULL) {
        AXK_LOGE(TAG, "Sec-WebSocket-Accept not found");
        return -1;
    }

    // See axk_crypto_sha1() arg size
    ...
    axk_crypto_sha1(expected_server_text, key_len, expected_server_sha1);
    axk_crypto_base64_encode(expected_server_key, sizeof(expected_server_key),  &outlen, expected_server_sha1, sizeof(expected_server_sha1));
    expected_server_key[ (outlen < sizeof(expected_server_key)) ? outlen : (sizeof(expected_server_key) - 1) ] = 0;
    AXK_LOGD(TAG, "server key=%s, send_key=%s, expected_server_key=%s", (char *)server_key, (char *)client_key, expected_server_key);
    if (strcmp((char *)expected_server_key, (char *)server_key) != 0) {
        AXK_LOGE(TAG, "Invalid websocket key");
        return -1;
    }
    return 0;
}

static int _ws_write(axk_transport_handle_t t, int opcode, int mask_flag, const char *b, int len, int timeout_ms)
{
    transport_ws_t *ws = axk_transport_get_context_data(t);
    char *buffer = (char *)b;
    ...
    int poll_write;
    if ((poll_write = axk_transport_poll_write(ws->parent, timeout_ms)) <= 0) {
        AXK_LOGE(TAG, "Error transport_poll_write");
        return poll_write;
    }
    ...
    if (axk_transport_write(ws->parent, ws_header, header_len, timeout_ms) != header_len) {
        AXK_LOGE(TAG, "Error write header");
        return -1;
    }
    if (len == 0) {
        return 0;
    ...
    int ret = axk_transport_write(ws->parent, buffer, len, timeout_ms);
...
int axk_transport_ws_send_raw(axk_transport_handle_t t, ws_transport_opcodes_t opcode, const char *b, int len, int timeout_ms)
{
    ...
    if (t == NULL) {
        AXK_LOGE(TAG, "Transport must be a valid ws handle");
        return AXK_ERR_INVALID_ARG;
    }
    AXK_LOGD(TAG, "Sending raw ws message with opcode %d", op_code);
    return _ws_write(t, op_code, WS_MASK, b, len, timeout_ms);
}

static int ws_write(axk_transport_handle_t t, const char *b, int len, int timeout_ms)
{
    if (len == 0) {
        // Default transport write of zero length in ws layer sends out a ping message.
        // This behaviour could however be altered in SDK 5.0, since a separate API for sending
        // messages with user defined opcodes has been introduced.
        AXK_LOGD(TAG, "Write PING message");
...
static int ws_read_payload(axk_transport_handle_t t, char *buffer, int len, int timeout_ms)
{
    transport_ws_t *ws = axk_transport_get_context_data(t);
    ...
    if (ws->frame_state.bytes_remaining > len) {
        AXK_LOGD(TAG, "Actual data to receive (%d) are longer than ws buffer (%d)", ws->frame_state.bytes_remaining, len);
        bytes_to_read = len;
    ...
    if (bytes_to_read != 0 && (rlen = axk_transport_read(ws->parent, buffer, bytes_to_read, timeout_ms)) <= 0) {
        AXK_LOGE(TAG, "Error read data");
        return rlen;
    }
...
static int ws_read_header(axk_transport_handle_t t, char *buffer, int len, int timeout_ms)
{
    transport_ws_t *ws = axk_transport_get_context_data(t);
    int payload_len;
    ...
    if ((poll_read = axk_transport_poll_read(ws->parent, timeout_ms)) <= 0) {
        return poll_read;
    }
    ...
    int header = 2;
    int mask_len = 4;
    if ((rlen = axk_transport_read(ws->parent, data_ptr, header, timeout_ms)) <= 0) {
        AXK_LOGE(TAG, "Error read data");
        return rlen;
    }
    ...
    if (mask) {
        // Read and store mask
        if (payload_len != 0 && (rlen = axk_transport_read(ws->parent, buffer, mask_len, timeout_ms)) <= 0) {
            AXK_LOGE(TAG, "Error read data");
            return rlen;
        }
...
static int ws_handle_control_frame_internal(axk_transport_handle_t t, int timeout_ms)
{
    transport_ws_t *ws = axk_transport_get_context_data(t);
    ...
    if (payload_len > WS_TRANSPORT_MAX_CONTROL_FRAME_BUFFER_LEN) {
        AXK_LOGE(TAG, "Not enough room for reading control frames (need=%d, max_allowed=%d)",
    ...
    if (control_frame_buffer_len > 0) {
        ...
        if (control_frame_buffer == NULL) {
            AXK_LOGE(TAG, "Cannot allocate buffer for control frames, need-%d", control_frame_buffer_len);
            return -1;
        }
    } else {
    ...
    if (actual_len != payload_len) {
        AXK_LOGE(TAG, "Control frame (opcode=%d) payload read failed (payload_len=%d, read_len=%d)",
                 ws->frame_state.opcode, payload_len, actual_len);
        ret = -1;
        goto free_payload_buffer;
    }

    ret = axk_transport_ws_handle_control_frames(t, control_frame_buffer, control_frame_buffer_len, timeout_ms, false);
...
static int ws_read(axk_transport_handle_t t, char *buffer, int len, int timeout_ms)
{
    int rlen = 0;
    transport_ws_t *ws = axk_transport_get_context_data(t);
    ...
    if (ws->frame_state.payload_len) {
        if ( (rlen = ws_read_payload(t, buffer, len, timeout_ms)) <= 0) {
            AXK_LOGE(TAG, "Error reading payload data");
            ws->frame_state.bytes_remaining = 0;
            return rlen;
        }
    }
...
static int ws_poll_read(axk_transport_handle_t t, int timeout_ms)
{
    transport_ws_t *ws = axk_transport_get_context_data(t);
    return axk_transport_poll_read(ws->parent, timeout_ms);
}

static int ws_poll_write(axk_transport_handle_t t, int timeout_ms)
{
    transport_ws_t *ws = axk_transport_get_context_data(t);
    return axk_transport_poll_write(ws->parent, timeout_ms);;
}

static int ws_close(axk_transport_handle_t t)
{
    transport_ws_t *ws = axk_transport_get_context_data(t);
    return axk_transport_close(ws->parent);
}

static axk_err_t ws_destroy(axk_transport_handle_t t)
{
    transport_ws_t *ws = axk_transport_get_context_data(t);
    free(ws->buffer);
    free(ws->path);
...
static axk_err_t internal_axk_transport_ws_set_path(axk_transport_handle_t t, const char *path)
{
    if (t == NULL) {
        return AXK_ERR_INVALID_ARG;
    }
...
{
    axk_err_t err = internal_axk_transport_ws_set_path(t, path);
    if (err != AXK_OK) {
        AXK_LOGE(TAG, "axk_transport_ws_set_path has internally failed with err=%d", err);
    }
}

static int ws_get_socket(axk_transport_handle_t t)
{
    if (t) {
...
axk_transport_handle_t axk_transport_ws_init(axk_transport_handle_t parent_handle)
{
    ...
    AXK_TRANSPORT_MEM_CHECK(TAG, ws, {
        axk_transport_destroy(t);
        return NULL;
    });
    ws->parent = parent_handle;

    ws->path = strdup("/");
    AXK_TRANSPORT_MEM_CHECK(TAG, ws->path, {
        free(ws);
        axk_transport_destroy(t);
        return NULL;
    });
    ws->buffer = malloc(WS_BUFFER_SIZE);
    AXK_TRANSPORT_MEM_CHECK(TAG, ws->buffer, {
        free(ws->path);
        free(ws);
        axk_transport_destroy(t);
        return NULL;
    ...
    axk_transport_set_func(t, ws_connect, ws_read, ws_write, ws_close, ws_poll_read, ws_poll_write, ws_destroy);
    // websocket underlying transfer is the payload transfer handle
    axk_transport_set_parent_transport_func(t, ws_get_payload_transport_handle);

    axk_transport_set_context_data(t, ws);
...
axk_err_t axk_transport_ws_set_config(axk_transport_handle_t t, const axk_transport_ws_config_t *config)
{
    ...
    transport_ws_t *ws = axk_transport_get_context_data(t);
    if (config->ws_path) {
        err = internal_axk_transport_ws_set_path(t, config->ws_path);
        AXK_TRANSPORT_ERR_OK_CHECK(TAG, err, return err;)
    }
    if (config->sub_protocol) {
        err = axk_transport_ws_set_subprotocol(t, config->sub_protocol);
        AXK_TRANSPORT_ERR_OK_CHECK(TAG, err, return err;)
    }
    if (config->user_agent) {
        err = axk_transport_ws_set_user_agent(t, config->user_agent);
        AXK_TRANSPORT_ERR_OK_CHECK(TAG, err, return err;)
    }
    if (config->headers) {
        err = axk_transport_ws_set_headers(t, config->headers);
        AXK_TRANSPORT_ERR_OK_CHECK(TAG, err, return err;)
    }
...
static int axk_transport_ws_handle_control_frames(axk_transport_handle_t t, char *buffer, int len, int timeout_ms, bool client_closed)
{
    ...
    AXK_LOGD(TAG, "Handling control frame with %d bytes payload", payload_len);
    if (payload_len > len) {
        AXK_LOGE(TAG, "Not enough room for processing the payload (need=%d, available=%d)", payload_len, len);
    ...
    if (ws->frame_state.opcode == WS_OPCODE_PING) {
        ...
        if (actual_len != payload_len) {
            AXK_LOGE(TAG, "PONG send failed (payload_len=%d, written_len=%d)", payload_len, actual_len);
            return -1;
        }
        AXK_LOGD(TAG, "PONG sent correctly (payload_len=%d)", payload_len);
        ...
        if (buffer && payload_len > 0) {     // if some payload, print out the status code
            ...
            AXK_LOGI(TAG, "Got CLOSE frame with status code=%u", ntohs(*code_network_order));
        }
        ...
        if (client_closed == false) {
            ...
            if (_ws_write(t, WS_OPCODE_CLOSE | WS_FIN, WS_MASK, NULL, 0, timeout_ms) < 0) {
                AXK_LOGE(TAG, "Sending CLOSE frame with 0 payload failed");
                return -1;
            }
            AXK_LOGD(TAG, "CLOSE frame with no payload sent correctly");
        }

        // control frame handled correctly, reset the flag indicating new header received
        ws->frame_state.header_received = false;
        int ret = axk_transport_ws_poll_connection_closed(t, timeout_ms);
        if (ret == 0) {
            AXK_LOGW(TAG, "Connection cannot be terminated gracefully within timeout=%d", timeout_ms);
            return -1;
        }
        if (ret < 0) {
            AXK_LOGW(TAG, "Connection terminated while waiting for clean TCP close");
            return -1;
        }
        AXK_LOGI(TAG, "Connection terminated gracefully");
        return 1;
    } else if (ws->frame_state.opcode == WS_OPCODE_PONG) {
        // handle PONG: just indicate return code
        AXK_LOGD(TAG, "Received PONG frame with payload=%d", payload_len);
        // control frame handled correctly, reset the flag indicating new header received
        ws->frame_state.header_received = false;
        return 2;
    }
...
int axk_transport_ws_poll_connection_closed(axk_transport_handle_t t, int timeout_ms)
{
    ...
    int sock = axk_transport_get_socket(t);
    fd_set readset;
    fd_set errset;
    FD_ZERO(&readset);
    FD_ZERO(&errset);
    ...
    int ret = select(sock + 1, &readset, NULL, &errset, axk_transport_utils_ms_to_timeval(timeout_ms, &timeout));
    if (ret > 0) {
        if (FD_ISSET(sock, &readset)) {
            ...
            AXK_LOGW(TAG, "axk_transport_ws_poll_connection_closed: unexpected data readable on socket=%d", sock);
        } else if (FD_ISSET(sock, &errset)) {
            int sock_errno = 0;
            uint32_t optlen = sizeof(sock_errno);
            getsockopt(sock, SOL_SOCKET, SO_ERROR, &sock_errno, &optlen);
            AXK_LOGD(TAG, "axk_transport_ws_poll_connection_closed select error %d, errno = %s, fd = %d", sock_errno, strerror(sock_errno), sock);
            ...
            AXK_LOGE(TAG, "axk_transport_ws_poll_connection_closed: unexpected errno=%d on socket=%d", sock_errno, sock);
        }
