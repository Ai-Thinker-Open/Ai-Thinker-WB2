...
static int ws_connect(axk_transport_handle_t t, const char *host, int port, int timeout_ms)
{
    ...
    if (axk_transport_connect(ws->parent, host, port, timeout_ms) < 0) {
        blog_error( "Error connecting to host %s:%d", host, port);
        return -1;
    }
    ...
    if (len <= 0 || len >= WS_BUFFER_SIZE) {
        blog_error( "Error in request generation, desired request len: %d, buffer size: %d", len, WS_BUFFER_SIZE);
        return -1;
    }
    if (ws->sub_protocol) {
        blog_debug( "sub_protocol: %s", ws->sub_protocol);
        int r = snprintf(ws->buffer + len, WS_BUFFER_SIZE - len, "Sec-WebSocket-Protocol: %s\r\n", ws->sub_protocol);
        len += r;
        if (r <= 0 || len >= WS_BUFFER_SIZE) {
            blog_error( "Error in request generation"
                     "(snprintf of subprotocol returned %d, desired request len: %d, buffer size: %d", r, len, WS_BUFFER_SIZE);
            return -1;
        }
    }
    if (ws->headers) {
        blog_debug( "headers: %s", ws->headers);
    ...
    blog_debug( "server key=%s, send_key=%s, expected_server_key=%s", (char *)server_key, (char *)client_key, expected_server_key);
    if (strcmp((char *)expected_server_key, (char *)server_key) != 0) {
        blog_error( "Invalid websocket key");
        return -1;
    }
    return 0;
}
...
static int _ws_write(axk_transport_handle_t t, int opcode, int mask_flag, const char *b, int len, int timeout_ms)
{
    ...
    if ((poll_write = axk_transport_poll_write(ws->parent, timeout_ms)) <= 0) {
        blog_error( "Error transport_poll_write");
        return poll_write;
    }
...
int axk_transport_ws_send_raw(axk_transport_handle_t t, ws_transport_opcodes_t opcode, const char *b, int len, int timeout_ms)
{
    ...
    if (t == NULL) {
        blog_error( "Transport must be a valid ws handle");
        return AXK_ERR_INVALID_ARG;
    }
    blog_debug( "Sending raw ws message with opcode %d", op_code);
...
static int ws_read_payload(axk_transport_handle_t t, char *buffer, int len, int timeout_ms)
{
    ...
    if (ws->frame_state.bytes_remaining > len) {
        blog_debug( "Actual data to receive (%d) are longer than ws buffer (%d)", ws->frame_state.bytes_remaining, len);
        bytes_to_read = len;
...
static int ws_handle_control_frame_internal(axk_transport_handle_t t, int timeout_ms)
{
    ...
    if (payload_len > WS_TRANSPORT_MAX_CONTROL_FRAME_BUFFER_LEN) {
        blog_error( "Not enough room for reading control frames (need=%d, max_allowed=%d)",
    ...
    if (control_frame_buffer_len > 0) {
        ...
        if (control_frame_buffer == NULL) {
            blog_error( "Cannot allocate buffer for control frames, need-%d", control_frame_buffer_len);
            return -1;
        }
    } else {
    ...
    if (actual_len != payload_len) {
        blog_error( "Control frame (opcode=%d) payload read failed (payload_len=%d, read_len=%d)",
...
static int ws_read(axk_transport_handle_t t, char *buffer, int len, int timeout_ms)
{
    ...
    if (ws->frame_state.payload_len) {
        if ( (rlen = ws_read_payload(t, buffer, len, timeout_ms)) <= 0) {
            blog_error( "Error reading payload data");
            ws->frame_state.bytes_remaining = 0;
            return rlen;
        }
    }
...
void axk_transport_ws_set_path(axk_transport_handle_t t, const char *path)
{
    ...
    if (err != AXK_OK) {
        blog_error( "axk_transport_ws_set_path has internally failed with err=%d", err);
    }
}
...
static int axk_transport_ws_handle_control_frames(axk_transport_handle_t t, char *buffer, int len, int timeout_ms, bool client_closed)
{
    ...
    blog_debug( "Handling control frame with %d bytes payload", payload_len);
    if (payload_len > len) {
        blog_error( "Not enough room for processing the payload (need=%d, available=%d)", payload_len, len);
    ...
    if (ws->frame_state.opcode == WS_OPCODE_PING) {
        ...
        if (actual_len != payload_len) {
            blog_error( "PONG send failed (payload_len=%d, written_len=%d)", payload_len, actual_len);
            return -1;
        }
        blog_debug( "PONG sent correctly (payload_len=%d)", payload_len);
        ...
        if (buffer && payload_len > 0) {     // if some payload, print out the status code
            ...
            blog_info( "Got CLOSE frame with status code=%u", ntohs(*code_network_order));
        }
        ...
        if (client_closed == false) {
            ...
            if (_ws_write(t, WS_OPCODE_CLOSE | WS_FIN, WS_MASK, NULL, 0, timeout_ms) < 0) {
                blog_error( "Sending CLOSE frame with 0 payload failed");
                return -1;
            }
            blog_debug( "CLOSE frame with no payload sent correctly");
        }
        ...
        if (ret == 0) {
            blog_warn( "Connection cannot be terminated gracefully within timeout=%d", timeout_ms);
            return -1;
        }
        if (ret < 0) {
            blog_warn( "Connection terminated while waiting for clean TCP close");
            return -1;
        }
        blog_info( "Connection terminated gracefully");
        return 1;
    } else if (ws->frame_state.opcode == WS_OPCODE_PONG) {
        // handle PONG: just indicate return code
        blog_debug( "Received PONG frame with payload=%d", payload_len);
        // control frame handled correctly, reset the flag indicating new header received
        ws->frame_state.header_received = false;
        return 2;
    }
...
int axk_transport_ws_poll_connection_closed(axk_transport_handle_t t, int timeout_ms)
{
    ...
    if (ret > 0) {
        if (FD_ISSET(sock, &readset)) {
            ...
            blog_warn( "axk_transport_ws_poll_connection_closed: unexpected data readable on socket=%d", sock);
        } else if (FD_ISSET(sock, &errset)) {
            int sock_errno = 0;
            uint32_t optlen = sizeof(sock_errno);
            getsockopt(sock, SOL_SOCKET, SO_ERROR, &sock_errno, &optlen);
            blog_debug( "axk_transport_ws_poll_connection_closed select error %d, errno = %s, fd = %d", sock_errno, strerror(sock_errno), sock);
            ...
            blog_error( "axk_transport_ws_poll_connection_closed: unexpected errno=%d on socket=%d", sock_errno, sock);
        }
