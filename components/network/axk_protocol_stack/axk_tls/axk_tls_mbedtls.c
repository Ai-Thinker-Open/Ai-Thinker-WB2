...
static void mbedtls_print_error_msg(int error)
{
    ...
    blog_info( "(%04X): %s", error, error_buf);
#endif
}
...
axk_err_t axk_create_mbedtls_handle(const char *hostname, size_t hostlen, const void *cfg, axk_tls_t *tls)
{
    ...
    if (tls->role == AXK_TLS_CLIENT) {
        ...
        if (axk_ret != AXK_OK) {
            blog_error( "Failed to set client configurations, returned [0x%04X] (%s)", axk_ret, axk_err_to_name(axk_ret));
            goto exit;
        }
        ...
        if (axk_ret != 0) {
            blog_error( "Failed to set server configurations, returned [0x%04X] (%s)", axk_ret, axk_err_to_name(axk_ret));
            goto exit;
        }
#else
            blog_error( "AXK_TLS_SERVER Not enabled in Kconfig");
            goto exit;
#endif
    }
    ...
    if ((ret = mbedtls_ctr_drbg_seed(&tls->ctr_drbg,
        ...
        blog_error( "mbedtls_ctr_drbg_seed returned -0x%04X", -ret);
    ...
    if ((ret = mbedtls_ssl_setup(&tls->ssl, &tls->conf)) != 0) {
        blog_error( "mbedtls_ssl_setup returned -0x%04X", -ret);
        mbedtls_print_error_msg(ret);
        AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
...
int axk_mbedtls_handshake(axk_tls_t *tls, const axk_tls_cfg_t *cfg)
{
    ...
    if (cfg->client_session != NULL) {
        blog_debug( "Reusing the already saved client session context");
        if ((ret = mbedtls_ssl_set_session(&tls->ssl, &(cfg->client_session->saved_session))) != 0 ) {
            blog_error( " mbedtls_ssl_conf_session returned -0x%04X", -ret);
            return -1;
        }
    }
#endif
    ...
    if (ret == 0) {
        ...
        if (ret != AXK_TLS_ERR_SSL_WANT_READ && ret != AXK_TLS_ERR_SSL_WANT_WRITE) {
            blog_error( "mbedtls_ssl_handshake returned -0x%04X", -ret);
            mbedtls_print_error_msg(ret);
            AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
...
ssize_t axk_mbedtls_read(axk_tls_t *tls, char *data, size_t datalen)
{
    ...
    if (ret < 0) {
        ...
        if (ret != AXK_TLS_ERR_SSL_WANT_READ  && ret != AXK_TLS_ERR_SSL_WANT_WRITE) {
            AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
            blog_error( "read error :-0x%04X:", -ret);
            mbedtls_print_error_msg(ret);
        }
    }
    return ret;
...
ssize_t axk_mbedtls_write(axk_tls_t *tls, const char *data, size_t datalen)
{
    ...
    while (written < datalen) {
        ...
        if (datalen > MBEDTLS_SSL_OUT_CONTENT_LEN) {
            blog_debug( "Fragmenting data of excessive size :%d, offset: %d, size %d", datalen, written, write_len);
        }
        ...
        if (ret <= 0) {
            if (ret != AXK_TLS_ERR_SSL_WANT_READ  && ret != AXK_TLS_ERR_SSL_WANT_WRITE && ret != 0) {
                ...
                blog_error( "write error :-0x%04X:", -ret);
                mbedtls_print_error_msg(ret);
                return ret;
            } else {
                // Exiting the tls-write process as less than desired datalen are writable
                blog_debug( "mbedtls_ssl_write() returned -0x%04X, already written %d, exitting...", -ret, written);
                mbedtls_print_error_msg(ret);
...
void axk_mbedtls_verify_certificate(axk_tls_t *tls)
{
    ...
    if ((flags = mbedtls_ssl_get_verify_result(&tls->ssl)) != 0) {
        blog_info( "Failed to verify peer certificate!");
        ...
        blog_debug( "verification info: %s", buf);
#endif
    } else {
...
{
    if (!tls) {
        blog_error( "empty arg passed to axk_tls_get_bytes_avail()");
        return AXK_FAIL;
    }
...
static axk_err_t set_ca_cert(axk_tls_t *tls, const unsigned char *cacert, size_t cacert_len)
{
    ...
    if (ret < 0) {
        blog_error( "mbedtls_x509_crt_parse returned -0x%04X", -ret);
        mbedtls_print_error_msg(ret);
        AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
    ...
    if (ret > 0) {
        ...
        blog_warn( "mbedtls_x509_crt_parse was partly successful. No. of failed certificates: %d", ret);
    }
...
static axk_err_t set_pki_context(axk_tls_t *tls, const axk_tls_pki_t *pki)
{
    ...
    if (pki->publiccert_pem_buf != NULL &&
        ...
        if (pki->axk_ds_data != NULL) {
            ...
            if (ret != AXK_OK) {
                blog_error( "Failed to initialize pk context for axk_ds");
                return ret;
            }
        } else
#endif
        ...
        if (ret < 0) {
            blog_error( "mbedtls_pk_parse_keyfile returned -0x%04X", -ret);
            mbedtls_print_error_msg(ret);
            AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
        ...
        if (ret < 0) {
            blog_error( "mbedtls_ssl_conf_own_cert returned -0x%04X", -ret);
            mbedtls_print_error_msg(ret);
            AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
...
static axk_err_t set_global_ca_store(axk_tls_t *tls)
{
    ...
    if (global_cacert == NULL) {
        blog_error( "global_cacert is NULL");
...
#ifndef NDEBUG
    if (ret != 0) {
        blog_error( "Writing session ticket resulted in error code -0x%04X", -ret);
        mbedtls_print_error_msg(ret);
    }
#endif
    return ret;
...
#ifndef NDEBUG
    if (ret != 0) {
        blog_debug( "Parsing session ticket resulted in error code -0x%04X", -ret);
        mbedtls_print_error_msg(ret);
    }
#endif
    return ret;
...
axk_err_t axk_mbedtls_server_session_ticket_ctx_init(axk_tls_server_session_ticket_ctx_t *ctx)
{
    ...
    if((ret = mbedtls_ssl_ticket_setup(&ctx->ticket_ctx,
        ...
        blog_error( "mbedtls_ssl_ticket_setup returned -0x%04X", -ret);
        mbedtls_print_error_msg(ret);
...
axk_err_t set_server_config(axk_tls_cfg_server_t *cfg, axk_tls_t *tls)
{
    ...
    if ((ret = mbedtls_ssl_config_defaults(&tls->conf,
        ...
        blog_error( "mbedtls_ssl_config_defaults returned -0x%04X", -ret);
        mbedtls_print_error_msg(ret);
        AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
    ...
    if (cfg->use_secure_element) {
...
#else
        blog_error( "Please enable secure element support for AXK-TLS in menuconfig");
        return AXK_FAIL;
#endif /* CONFIG_AXK_TLS_USE_SECURE_ELEMENT */
        ...
        if (axk_ret != AXK_OK) {
            blog_error( "Failed to set server pki context");
            return axk_ret;
        }
    } else {
        blog_error( "Missing server certificate and/or key");
        return AXK_ERR_INVALID_STATE;
    }

#ifdef CONFIG_AXK_TLS_SERVER_SESSION_TICKETS
    if (cfg->ticket_ctx) {
        blog_debug( "Enabling server-side tls session ticket support");
...
axk_err_t set_client_config(const char *hostname, size_t hostlen, axk_tls_cfg_t *cfg, axk_tls_t *tls)
{
    ...
    if (!cfg->skip_common_name) {
        ...
        if ((ret = mbedtls_ssl_set_hostname(&tls->ssl, use_host)) != 0) {
            blog_error( "mbedtls_ssl_set_hostname returned -0x%04X", -ret);
            mbedtls_print_error_msg(ret);
            AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
            free(use_host);
    ...
    if (cfg->alpn_protos) {
        ...
        if ((ret = mbedtls_ssl_conf_alpn_protocols(&tls->conf, cfg->alpn_protos)) != 0) {
            blog_error( "mbedtls_ssl_conf_alpn_protocols returned -0x%04X", -ret);
            AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
            mbedtls_print_error_msg(ret);
            return AXK_ERR_MBEDTLS_SSL_CONF_ALPN_PROTOCOLS_FAILED;
        }
#else
        blog_error( "alpn_protos configured but not enabled in menuconfig: Please enable MBEDTLS_SSL_ALPN option");
        return AXK_ERR_INVALID_STATE;
#endif
    }

#ifdef CONFIG_AXK_TLS_CLIENT_SESSION_TICKETS
    blog_debug( "Enabling client-side tls session ticket support");
    ...
    if (cfg->crt_bundle_attach != NULL) {
#ifdef CONFIG_MBEDTLS_CERTIFICATE_BUNDLE
        blog_debug( "Use certificate bundle");
        mbedtls_ssl_conf_authmode(&tls->conf, MBEDTLS_SSL_VERIFY_REQUIRED);
        cfg->crt_bundle_attach(&tls->conf);
#else //CONFIG_MBEDTLS_CERTIFICATE_BUNDLE
        blog_error( "use_crt_bundle configured but not enabled in menuconfig: Please enable MBEDTLS_CERTIFICATE_BUNDLE option");
        return AXK_ERR_INVALID_STATE;
#endif
        ...
        blog_debug( "ssl psk authentication");
        ...
        if (ret != 0) {
            blog_error( "mbedtls_ssl_conf_psk returned -0x%04X", -ret);
            mbedtls_print_error_msg(ret);
            AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
            return AXK_ERR_MBEDTLS_SSL_CONF_PSK_FAILED;
        }
#else
        blog_error( "psk_hint_key configured but not enabled in menuconfig: Please enable AXK_TLS_PSK_VERIFICATION option");
        return AXK_ERR_INVALID_STATE;
#endif
#ifdef CONFIG_AXK_TLS_CLIENT_SESSION_TICKETS
    } else if (cfg->client_session != NULL) {
        blog_debug( "Resuing the saved client session");
#endif
    } else {
#ifdef CONFIG_AXK_TLS_SKIP_SERVER_CERT_VERIFY
        mbedtls_ssl_conf_authmode(&tls->conf, MBEDTLS_SSL_VERIFY_NONE);
#else
        blog_error( "No server verification option set in axk_tls_cfg_t structure. Check axk_tls API reference");
    ...
    if (cfg->use_secure_element) {
        ...
        if (cfg->clientcert_pem_buf == NULL) {
            blog_error( "Client certificate is also required with the DS parameters");
            return AXK_ERR_INVALID_STATE;
        }
        ...
        if (axk_ret != AXK_OK) {
            blog_error( "Failed to set client pki context for the DS peripheral, returned [0x%04X] (%s)", ret, axk_err_to_name(axk_ret));
            return axk_ret;
        }
#else
        blog_error( "Please enable the DS peripheral support for the AXK-TLS in menuconfig. (only supported for the BL602 chip)");
        return AXK_FAIL;
#endif
        ...
        axk_err_t axk_ret = set_pki_context(tls, &pki);
        if (axk_ret != AXK_OK) {
            blog_error( "Failed to set client pki context");
            return axk_ret;
        }
    } else if (cfg->clientcert_buf != NULL || cfg->clientkey_buf != NULL) {
        blog_error( "You have to provide both clientcert_buf and clientkey_buf for mutual authentication");
        return AXK_ERR_INVALID_STATE;
    }
    return AXK_OK;
}
...
int axk_mbedtls_server_session_create(axk_tls_cfg_server_t *cfg, int sockfd, axk_tls_t *tls)
{
    ...
    if (axk_ret != AXK_OK) {
        blog_error( "create_ssl_handle failed, returned [0x%04X] (%s)", axk_ret, axk_err_to_name(axk_ret));
...
axk_err_t axk_mbedtls_init_global_ca_store(void)
{
    if (global_cacert == NULL) {
        ...
        if (global_cacert == NULL) {
            blog_error( "global_cacert not allocated");
            return AXK_ERR_NO_MEM;
        }
...
axk_err_t axk_mbedtls_set_global_ca_store(const unsigned char *cacert_pem_buf, const unsigned int cacert_pem_bytes)
{
    ...
    blog_error( "Please disable dynamic freeing of ca cert in mbedtls (CONFIG_MBEDTLS_DYNAMIC_FREE_CA_CERT)\n in order to use the global ca_store");
    return AXK_FAIL;
#endif
    if (cacert_pem_buf == NULL) {
        blog_error( "cacert_pem_buf is null");
        return AXK_ERR_INVALID_ARG;
    }
    int ret;
    if (global_cacert == NULL) {
    ...
    if (ret < 0) {
        ...
        blog_error( "mbedtls_x509_crt_parse was partly successful. No. of failed certificates: %d", ret);
...
static axk_err_t axk_init_atecc608a(uint8_t i2c_addr)
{
    ...
    if(ret != 0) {
        blog_error( "Failed to initialize atca device, returned -0x%04X", -ret);
        return AXK_FAIL;
    }
    return AXK_OK;
}
...
static axk_err_t axk_set_atecc608a_pki_context(axk_tls_t *tls, const void *pki)
{
    int ret = 0;
    axk_err_t axk_ret = AXK_FAIL;
    blog_info( "Initialize the ATECC interface...");
    (void)axk_ret;
    (void)cert_def;
    ...
    if (ret != 0) {
        blog_error( "Failed to get device cert def");
        return AXK_ERR_AXK_TLS_SE_FAILED;
    }
    ...
    if (ret != 0) {
        blog_error( "Failed to parse cert from device, return 0x%04X", ret);
        mbedtls_print_error_msg(ret);
        return AXK_ERR_AXK_TLS_SE_FAILED;
    }
    ...
    if(cfg->clientcert_buf != NULL) {
        ...
        blog_error( "Device certificate must be provided for TrustCustom Certs");
        return AXK_FAIL;
    }
#endif /* CONFIG_ATECC608A_TCUSTOM */
    ret = atca_mbedtls_pk_init(&tls->clientkey, 0);
    if (ret != 0) {
        blog_error( "Failed to parse key from device");
        AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
        mbedtls_print_error_msg(ret);
        return AXK_ERR_AXK_TLS_SE_FAILED;
    }
    ...
    if (ret != 0) {
        blog_error( "Failed to configure client cert, returned -0x%04X", ret);
        mbedtls_print_error_msg(ret);
        AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_MBEDTLS, -ret);
        return AXK_ERR_AXK_TLS_SE_FAILED;
    }
...
static axk_err_t axk_mbedtls_init_pk_ctx_for_ds(const void *pki)
{
    ...
    if ((ret = mbedtls_pk_setup_rsa_alt(((const axk_tls_pki_t*)pki)->pk_key, &rsakey, NULL, axk_ds_rsa_sign,
                                        axk_ds_get_keylen )) != 0) {
        blog_error( "Error in mbedtls_pk_setup_rsa_alt, returned -0x%04X", -ret);
        mbedtls_print_error_msg(ret);
        return AXK_FAIL;
    }
    ret = axk_ds_init_data_ctx(((const axk_tls_pki_t*)pki)->axk_ds_data);
    if (ret != AXK_OK) {
        blog_error( "Failed to initialize DS parameters from nvs");
        return AXK_FAIL;
    }
    blog_debug( "DS peripheral params initialized.");
    return AXK_OK;
}
#endif /* CONFIG_AXK_TLS_USE_DS_PERIPHERAL */
