...
static axk_err_t axk_tls_hostname_to_fd(const char *host, size_t hostlen, int port, struct sockaddr_storage *address, int* fd)
{
    ...
    blog_debug( "host:%s: strlen %lu", use_host, (unsigned long)hostlen);
    int res = getaddrinfo(use_host, NULL, &hints, &address_info);
    if (res != 0 || address_info == NULL) {
        blog_error( "couldn't get hostname for :%s: "
    ...
    *fd = socket(address_info->ai_family, address_info->ai_socktype, address_info->ai_protocol);
    if (*fd < 0) {
        blog_error( "Failed to create socket (family %d socktype %d protocol %d)", address_info->ai_family, address_info->ai_socktype, address_info->ai_protocol);
    ...
    if (address_info->ai_family == AF_INET) {
        ...
        blog_debug( "[sock=%d] Resolved IPv4 address: %s", *fd, ipaddr_ntoa((const ip_addr_t*)&p->sin_addr.s_addr));
        ...
        blog_debug( "[sock=%d] Resolved IPv6 address: %s", *fd, ip6addr_ntoa((const ip6_addr_t*)&p->sin6_addr));
        memcpy(address, p, sizeof(struct sockaddr_in6 ));
    }
#endif
    else {
        blog_error( "Unsupported protocol family %d", address_info->ai_family);
        close(*fd);
        freeaddrinfo(address_info);
    ...
    if (cfg) {
        if (cfg->timeout_ms >= 0) {
            ...
            if (setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) != 0) {
                blog_error( "Fail to setsockopt SO_RCVTIMEO");
                return AXK_ERR_AXK_TLS_SOCKET_SETOPT_FAILED;
            }
            if (setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv)) != 0) {
                blog_error( "Fail to setsockopt SO_SNDTIMEO");
                return AXK_ERR_AXK_TLS_SOCKET_SETOPT_FAILED;
            }
        }
...
static axk_err_t axk_tls_set_socket_non_blocking(int fd, bool non_blocking)
{
    ...
    if ((flags = fcntl(fd, F_GETFL, NULL)) < 0) {
        blog_error( "[sock=%d] get file flags error: %s", fd, strerror(errno));
        return AXK_ERR_AXK_TLS_SOCKET_SETOPT_FAILED;
    }
    ...
    if (fcntl(fd, F_SETFL, flags) < 0) {
        blog_error( "[sock=%d] set blocking/nonblocking error: %s", fd, strerror(errno));
        return AXK_ERR_AXK_TLS_SOCKET_SETOPT_FAILED;
    }
    return AXK_OK;
}
...
static inline axk_err_t tcp_connect(const char *host, int hostlen, int port, const axk_tls_cfg_t *cfg, axk_tls_error_handle_t error_handle, int *sockfd)
{
    ...
    blog_debug( "[sock=%d] Connecting to server. HOST: %s, Port: %d", fd, host, port);
    if (connect(fd, (struct sockaddr *)&address, sizeof(struct sockaddr)) < 0) {
        if (errno == EINPROGRESS) {
            ...
            if (res < 0) {
                blog_error( "[sock=%d] select() error: %s", fd, strerror(errno));
                AXK_INT_EVENT_TRACKER_CAPTURE(error_handle, AXK_TLS_ERR_TYPE_SYSTEM, errno);
                goto err;
            }
            else if (res == 0) {
                blog_error( "[sock=%d] select() timeout", fd);
                ...
                if (getsockopt(fd, SOL_SOCKET, SO_ERROR, (void*)(&sockerr), &len) < 0) {
                    blog_error( "[sock=%d] getsockopt() error: %s", fd, strerror(errno));
                    ret = AXK_ERR_AXK_TLS_SOCKET_SETOPT_FAILED;
                    goto err;
                }
                else if (sockerr) {
                    AXK_INT_EVENT_TRACKER_CAPTURE(error_handle, AXK_TLS_ERR_TYPE_SYSTEM, sockerr);
                    blog_error( "[sock=%d] delayed connect error: %s", fd, strerror(sockerr));
                    goto err;
                }
            }
        } else {
            blog_error( "[sock=%d] connect() error: %s", fd, strerror(errno));
            goto err;
        }
    }
...
{
    if (!tls) {
        blog_error( "empty axk_tls parameter");
        return -1;
    }
    axk_err_t axk_ret;
    ...
    case AXK_TLS_INIT:
        ...
        if (tls->is_tls == false) {
            ...
            blog_debug( "non-tls connection established");
            return 1;
        }
        if (cfg && cfg->non_block) {
            FD_ZERO(&tls->rset);
    ...
    case AXK_TLS_CONNECTING:
        if (cfg && cfg->non_block) {
            ...
            if (FD_ISSET(tls->sockfd, &tls->rset) || FD_ISSET(tls->sockfd, &tls->wset)) {
                ...
                if (getsockopt(tls->sockfd, SOL_SOCKET, SO_ERROR, &error, &len) < 0) {
                    blog_debug( "Non blocking connect failed");
        ...
        if (axk_ret != AXK_OK) {
            blog_error( "create_ssl_handle failed");
            AXK_INT_EVENT_TRACKER_CAPTURE(tls->error_handle, AXK_TLS_ERR_TYPE_AXK, axk_ret);
            tls->conn_state = AXK_TLS_FAIL;
            return -1;
        }
    ...
    case AXK_TLS_HANDSHAKE:
        blog_debug( "handshake in progress...");
        return axk_tls_handshake(tls, cfg);
        break;
    case AXK_TLS_FAIL:
        blog_error( "failed to open a new connection");;
        break;
    default:
        blog_error( "invalid axk-tls state");
        break;
    }
    return -1;
}
...
int axk_tls_conn_new_sync(const char *hostname, int hostlen, int port, const axk_tls_cfg_t *cfg, axk_tls_t *tls)
{
    ...
    while (1) {
        ...
        if (ret == 1) {
            ...
            blog_error( "Failed to open new connection");
            return -1;
            ...
            if (expired >= timeout_ticks) {
                blog_warn( "Failed to open new connection in specified timeout");
...
{
    if (!tls || !sockfd) {
        blog_error( "Invalid arguments passed");
        return AXK_ERR_INVALID_ARG;
    }
    *sockfd = tls->sockfd;
    return AXK_OK;
