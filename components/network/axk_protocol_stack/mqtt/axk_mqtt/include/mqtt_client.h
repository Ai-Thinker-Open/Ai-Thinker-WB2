 ...
 *
 * User event handler receives context data in `axk_mqtt_event_t` structure with
 *  - `user_context` - user data from `axk_mqtt_client_config_t`
...
typedef enum {
    MQTT_EVENT_ANY = -1,
    MQTT_EVENT_ERROR = 0,          /*!< on error event, additional context: connection return code, error handle from axk_tls (if supported) */
 ...
 * including AXK-TLS error, but in the past only the errors from MQTT_ERROR_TYPE_AXK_TLS layer
 * were reported, so the AXK-TLS error type is re-defined here for backward compatibility
 */
#define MQTT_ERROR_TYPE_AXK_TLS MQTT_ERROR_TYPE_TCP_TRANSPORT
 ...
 *
 * Important: This structure extends `axk_tls_last_error` error structure and is backward compatible with it
 * (so might be down-casted and treated as `axk_tls_last_error` error, but recommended to update applications if used this way previously)
 *
 ...
 * | MQTT_ERROR_TYPE_TCP_TRANSPORT | axk_tls_last_axk_err, axk_tls_stack_err, axk_tls_cert_verify_flags, sock_errno | Error reported from tcp_transport/axk-tls |
...
typedef struct axk_mqtt_error_codes {
    /* compatible portion of the struct corraxkonding to struct axk_tls_last_error */
    axk_err_t axk_tls_last_axk_err;              /*!< last axk_err code reported from axk-tls component */
    int       axk_tls_stack_err;                 /*!< tls specific error code reported from underlying tls stack */
    int       axk_tls_cert_verify_flags;         /*!< tls flags reported from underlying tls stack during certificate verification */
    /* axk-mqtt specific structure extension */
    axk_mqtt_error_type_t error_type;            /*!< error type referring to the source of the error */
    axk_mqtt_connect_return_code_t connect_return_code; /*!< connection refused error code reported from MQTT broker on connection */
    /* tcp_transport extension */
    int       axk_transport_sock_errno;         /*!< errno from the underlying socket */
 ...
 */
typedef struct {
    axk_mqtt_event_id_t event_id;       /*!< MQTT event type */
    axk_mqtt_client_handle_t client;    /*!< MQTT client handle for this event */
    ...
    axk_mqtt_error_codes_t *error_handle; /*!< axk-mqtt error handle including axk-tls errors as well as internal mqtt errors */
...
typedef struct {
    ...
    axk_event_loop_handle_t event_loop_handle; /*!< handle for MQTT event loop library */
    ...
    const char *client_id;                  /*!< Set client id.
                                                 ...
                                                 Default client id is ``BL602_%CHIPID%`` where %CHIPID% are last 3 bytes of MAC address in hex format */
    ...
    axk_mqtt_transport_t transport;         /*!< overrides URI transport */
    int refresh_connection_after_ms;        /*!< Refresh connection after this value (in milliseconds) */
    const struct psk_key_hint *psk_hint_key;     /*!< Pointer to PSK struct defined in axk_tls.h to enable PSK authentication (as alternative to certificate verification). If not NULL and server/client certificates are NULL, PSK is enabled */
    bool          use_global_ca_store;      /*!< Use a global ca_store for all the connections in which this bool is set. */
    axk_err_t (*crt_bundle_attach)(void *conf); /*!< Pointer to AXK x509 Certificate Bundle attach function for the usage of certification bundles in mqtts */
    ...
    axk_mqtt_protocol_ver_t protocol_ver;   /*!< MQTT protocol version used for connection, defaults to value from menuconfig*/
 ...
 * configured in axk_mqtt_client_init
 *
 * @param client    mqtt client handle
 * @param uri
 *
 * @return AXK_FAIL if URI parse error, AXK_OK on success
 */
 ...
 *         AXK_ERR_INVALID_ARG on wrong initialization
 *         AXK_FAIL on other error
 */
 ...
 * @return AXK_OK on success
 *         AXK_ERR_INVALID_ARG on wrong initialization
 *         AXK_FAIL if client is in invalid state
 */
 ...
 * - It is thread safe, please refer to `axk_mqtt_client_subscribe` for details
 *
 * @param client    mqtt client handle
 * @param topic
 *
 ...
 * to the network is performed in the mqtt-task context (in contrast to the axk_mqtt_client_publish()
 * which sends the publish message immediately in the user task's context).
 * Thus, it could be used as a non blocking version of axk_mqtt_client_publish().
 *
 * @param client    mqtt client handle
 * @param topic     topic string
 * @param data      payload string (set to NULL, sending empty payload message)
 ...
 * @param config    mqtt configuration structure
 *
 * @return AXK_ERR_NO_MEM if failed to allocate
 *         AXK_ERR_INVALID_ARG if conflicts on transport configuration.
 *         AXK_OK on success
 */
