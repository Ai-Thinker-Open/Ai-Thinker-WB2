 ...
 *
 * Log library stores all tags provided to axk_log_level_set as a linked
...
typedef struct uncached_tag_entry_ {
    ...
    uint8_t level;  // axk_log_level_t as uint8_t
    char tag[0];    // beginning of a zero-terminated string
} uncached_tag_entry_t;

axk_log_level_t axk_log_default_level = CONFIG_LOG_DEFAULT_LEVEL;
...
static inline bool get_cached_log_level(const char *tag, axk_log_level_t *level);
static inline bool get_uncached_log_level(const char *tag, axk_log_level_t *level);
static inline void add_to_cache(const char *tag, axk_log_level_t level);
static void heap_bubble_down(int index);
static inline void heap_swap(int i, int j);
static inline bool should_output(axk_log_level_t level_for_message, axk_log_level_t level_for_tag);
...
/* Common code for getting the log level from cache, axk_log_impl_lock()
   should be called before calling this function. The function unlocks,
   as indicated in the name.
*/
static axk_log_level_t s_log_level_get_and_unlock(const char *tag)
{
    axk_log_level_t level_for_tag;
    ...
    if (!get_cached_log_level(tag, &level_for_tag)) {
        if (!get_uncached_log_level(tag, &level_for_tag)) {
            level_for_tag = axk_log_default_level;
        }
...
void axk_log_writev(axk_log_level_t level,
                   const char *tag,
                   const char *format,
                   va_list args)
{
    if (!axk_log_impl_lock_timeout()) {
        return;
    }
    axk_log_level_t level_for_tag = s_log_level_get_and_unlock(tag);
...
void axk_log_write(axk_log_level_t level,
                   const char *tag,
                   const char *format, ...)
{
    va_list list;
    va_start(list, format);
    axk_log_writev(level, tag, format, list);
    va_end(list);
}
...
static inline bool get_cached_log_level(const char *tag, axk_log_level_t *level)
{
    ...
    *level = (axk_log_level_t) s_log_cache[i].level;
