...
#ifndef CONFIG_SDK_TARGET_LINUX
#include "axk_memory_utils.h"  // for axk_ptr_byte_accessible
#else
static inline bool axk_ptr_byte_accessible(const void* ptr) {
    (void) ptr;
    return true;
}
#endif // CONFIG_SDK_TARGET_LINUX

//print number of bytes per line for axk_log_buffer_char and axk_log_buffer_hex
#define BYTES_PER_LINE 16

void axk_log_buffer_hex_internal(const char *tag, const void *buffer, uint16_t buff_len,
                                 axk_log_level_t log_level)
{
    if (buff_len == 0) {
        return;
    }
    ...
    do {
        ...
        if (!axk_ptr_byte_accessible(buffer)) {
        ...
        AXK_LOG_LEVEL(log_level, tag, "%s", hex_buffer);
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}

void axk_log_buffer_char_internal(const char *tag, const void *buffer, uint16_t buff_len,
                                  axk_log_level_t log_level)
{
    if (buff_len == 0) {
        return;
    ...
    do {
        ...
        AXK_LOG_LEVEL(log_level, tag, "%s", char_buffer);
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}
...
void axk_log_buffer_hexdump_internal(const char *tag, const void *buffer, uint16_t buff_len, axk_log_level_t log_level)
{
    ...
    do {
        ...
        AXK_LOG_LEVEL(log_level, tag, "%s", hd_buffer);
        buffer += bytes_cur_line;
        buff_len -= bytes_cur_line;
    } while (buff_len);
}
