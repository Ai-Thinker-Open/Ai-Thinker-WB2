...
char *axk_log_system_timestamp(void)
{
    ...
    if (xTaskGetSchedulerState() == taskSCHEDULER_NOT_STARTED) {
        uint32_t timestamp = axk_log_early_timestamp();
...
{
    if (unlikely(xTaskGetSchedulerState() == taskSCHEDULER_NOT_STARTED)) {
        return axk_log_early_timestamp();
    }
    static uint32_t base = 0;
    if (base == 0 && xPortGetCoreID() == 0) {
        base = axk_log_early_timestamp();
    }
...
{
#if CONFIG_SDK_TARGET_BL602
    /* BL602 ROM stores separate clock rate values for each CPU, but we want the PRO CPU value always */
