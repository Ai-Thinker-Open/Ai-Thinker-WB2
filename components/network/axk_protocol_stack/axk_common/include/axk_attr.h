 ...
 */
#ifndef __AXK_ATTR_H__
#define __AXK_ATTR_H__
...
#define IRAM_8BIT_ACCESSIBLE (CONFIG_SDK_TARGET_BL602 && CONFIG_BL602_IRAM_AS_8BIT_ACCESSIBLE_MEMORY)

// Make sure that IRAM is accessible as an 8-bit memory on BL602.
...
// Use as axk_rom_printf(DRAM_STR("Hello world!\n"));
...
/* Use SDK_DEPRECATED attribute to mark anything deprecated from use in
   AXK-SDK's own source code, but not deprecated for external users.
*/
#ifdef SDK_CI_BUILD
#define SDK_DEPRECATED(REASON) __attribute__((deprecated(REASON)))
#else
#define SDK_DEPRECATED(REASON)
#endif

#ifdef __cplusplus
}
#endif
#endif /* __AXK_ATTR_H__ */
