...
void btc_blufi_send_encap(uint8_t type, uint8_t* data, int total_data_len)
{
    ...
    {
        blog_info("blufi connection has been disconnected ");
        return;
    }
    ...
    while (remain_len > 0)
    {
        ...
        {
            if ((blufi_env.sec_mode & BLUFI_CTRL_SEC_MODE_CHECK_MASK) && (blufi_env.cbs && blufi_env.cbs->checksum_func))
            {
        ...
        {
            if ((blufi_env.sec_mode & BLUFI_DATA_SEC_MODE_CHECK_MASK) && (blufi_env.cbs && blufi_env.cbs->checksum_func))
            {
                hdr->fc |= BLUFI_FC_CHECK;
                checksum = blufi_env.cbs->checksum_func(hdr->seq, &hdr->seq, hdr->data_len + 2);
                memcpy(&hdr->data[hdr->data_len], &checksum, 2);
            ...
            if ((blufi_env.sec_mode & BLUFI_DATA_SEC_MODE_ENC_MASK) && (blufi_env.cbs && blufi_env.cbs->encrypt_func))
            {
                ret = blufi_env.cbs->encrypt_func(hdr->seq, hdr->data, hdr->data_len);
                if (ret == hdr->data_len)
                { /* enc must be success and enc len must equal to plain len */
                    hdr->fc |= BLUFI_FC_ENC;
                }
                else
                {
                    blog_error("%s encrypt error %d", __func__, ret);
...
static uint8_t btc_blufi_profile_init(void)
{
    ...
    {
        blog_info("BLUFI already initialized");
        return _BLUFI_ERROR;
    }
...
// {
//     if (!blufi_env.enabled) {
//         blog_info("BLUFI already de-initialized");
//         return _BLUFI_ERROR;
//     }
...
void btc_blufi_send_notify(uint8_t* pkt, int pkt_len)
{
    ...
    blog_info(" hdr = %d , pkt_len= %d", pkt, pkt_len);
...
void btc_blufi_recv_handler(uint8_t* data, int len)
{
    ...
    {
        blog_info("%s seq %d is not expect %d", __func__, hdr->seq, blufi_env.recv_seq + 1);
    ...
    if (BLUFI_FC_IS_ENC(hdr->fc) && (blufi_env.cbs && blufi_env.cbs->decrypt_func))
    {
        ...
        if (ret != hdr->data_len)
        { /* enc must be success and enc len must equal to plain len */
            blog_error("%s decrypt error %d", __func__, ret);
    ...
    if (BLUFI_FC_IS_CHECK(hdr->fc) && (blufi_env.cbs && blufi_env.cbs->checksum_func))
    {
        ...
        {
            blog_error("%s checksum error %04x, pkt %04x", __func__, checksum, checksum_pkt);
    ...
    if (BLUFI_FC_IS_FRAG(hdr->fc))
    {
        if (blufi_env.offset == 0)
        {
            ...
            {
                blog_error("%s no mem, len %d", __func__, blufi_env.total_len);
        ...
        {
            blog_info("%s payload is longer than packet length, len %d ", __func__, blufi_env.total_len);
...
void btc_blufi_cb_deep_copy(btc_msg_t* msg, void* p_dest, void* p_src)
{
    ...
    switch (msg->act)
    {
        case AXK_BLUFI_EVENT_RECV_STA_SSID:
            ...
            {
                blog_error("[BLUFI] %s %d no mem", __func__, msg->act);
            }
...
void btc_blufi_call_deep_copy(btc_msg_t* msg, void* p_dest, void* p_src)
{
    ...
    switch (msg->act)
    {
        ...
        case BTC_BLUFI_ACT_SEND_CUSTOM_DATA:
        {
            ...
            if (data == NULL)
            {
                blog_info("custom data is NULL");
                break;
            }
            dst->custom_data.data = malloc(src->custom_data.data_len);
            if (dst->custom_data.data == NULL)
            {
                blog_info("custom data malloc error");
                break;
            }
            memcpy(dst->custom_data.data, src->custom_data.data, src->custom_data.data_len);
            break;
...
static void btc_blufi_send_error_info(uint8_t state)
{
    ...
    *p++ = state;
    if (p - data > data_len)
    {
        blog_error("%s len error %d %d", __func__, (int)(p - data), data_len);
    }
...
static void btcaxk_blufi_send_wifi_list(uint16_t apCount, _blufi_ap_record_t* list)
{
    ...
    if (data == NULL)
    {
        blog_info("malloc error\n");
        return;
    }
    if (list == NULL || apCount == 0)
    {
        blog_error("%s value or value len error", __func__);
        return;
    }
    ...
    for (int i = 0; i < apCount; ++i)
    {
        ...
        {
            blog_info("%s len error, data_len: %d, data_len_diff: %d, malloc_size: %d", __func__, data_len, (data_len + len + 1 + 1), malloc_size);
            free(data);
            return;
        }
...
static void btc_blufi_send_custom_data(uint8_t* value, uint32_t value_len)
{
    ...
    if (data == NULL)
    {
        blog_error("%s mem malloc error", __func__);
        return;
    }
...
void btc_blufi_call_handler(btc_msg_t* msg)
{
    ...
    switch (msg->act)
    {
        ...
        case BTC_BLUFI_ACT_SEND_CUSTOM_DATA:
            ...
            break;
        default:
            blog_error("[blufi]%s UNKNOWN %d", __func__, msg->act);
            break;
    }
