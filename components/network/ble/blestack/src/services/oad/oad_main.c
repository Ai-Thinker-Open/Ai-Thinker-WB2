...
static u8_t oad_image_data_handler(struct bt_conn* conn, const u8_t* data, u16_t len)
{
    ...
    if (wData.wdata_buf) {
        ...
        // BT_WARN("left_size (0x%d) wData.index (0x%d) len (%d)\r\n", left_size, wData.index, len);
    ...
    uint8_t old_progress = (uint8_t)(((double)oad_env.upgrd_offset/oad_env.upgrd_file_size)*100.0);
    oad_env.upgrd_offset += len;
    uint8_t new_progress = (uint8_t)(((double)oad_env.upgrd_offset/oad_env.upgrd_file_size)*100.0);

    if (new_progress>old_progress)
        printf("left_size (0x%04x) wData.index (0x%04x) len (%d) progress:%d%%\r\n", left_size, wData.index, len, new_progress);
