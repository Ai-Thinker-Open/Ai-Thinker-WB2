...
static void update_tsen_value(hosal_adc_ctx_t *pstctx) {
  ...
  if (v0 > v1) {
    pstctx->tsen_data =
        (((float)v0 - (float)v1) - (float)(pstctx->tsen_offset)) / 7.753;
  } else {
    pstctx->tsen_data =
        (((float)v1 - (float)v0) - (float)(pstctx->tsen_offset)) / 7.753;
  }
}
#endif
...
static int adc_check_gpio_valid(GLB_GPIO_Type pin) {
  ...
  GLB_GPIO_Type gpio_arr[ADC_GPIO_NUM] = {
      GLB_GPIO_PIN_4,  GLB_GPIO_PIN_5,  GLB_GPIO_PIN_6,  GLB_GPIO_PIN_9,
      GLB_GPIO_PIN_10, GLB_GPIO_PIN_11, GLB_GPIO_PIN_12, GLB_GPIO_PIN_13,
      GLB_GPIO_PIN_14, GLB_GPIO_PIN_15,
  };
...
static void adc_init(hosal_adc_dev_t *adc) {
  ...
  if (mode == HOSAL_ADC_ONE_SHOT) {
    ...
    ADC_Scan_Channel_Config(pos_chlist_single, neg_chlist_single,
                            ADC_CHANNEL_MAX, ENABLE);
  } else {
...
int hosal_adc_value_get(hosal_adc_dev_t *adc, uint32_t channel,
  ...
  while ((val = adc_parse_data(pstctx->channel_data, ADC_CHANNEL_MAX,
                               channel)) == -1) {
    if (timeout-- == 0) {
      return -1;
    }
  ...
  if (adc_refer_channel < 12 && adc_refer_channel >= 0) {
    while ((refer_val = adc_parse_data(pstctx->channel_data, ADC_CHANNEL_MAX,
                                       adc_refer_channel)) == -1) {
      if (refer_timeout-- == 0) {
        return -1;
      }
...
int hosal_adc_stop(hosal_adc_dev_t *adc) { return 0; }
