...
static void _dump_partition(void)
{
    ...
    for (i = 0; i < part->ptTable.entryCnt; i++) {
        blog_info("[%02d]  %02u     %u         %u      %8s  %p  %p  %p  %p  %lu", i, part->ptEntries[i].type,
                                        part->ptEntries[i].device,
                                         part->ptEntries[i].activeIndex,
                                         part->ptEntries[i].name,
                                         (void*)(part->ptEntries[i].Address[0]),
                                         (void*)(part->ptEntries[i].Address[1]),
                                         (void*)(part->ptEntries[i].maxLen[0]),
                                        (void*)(part->ptEntries[i].maxLen[1]),
