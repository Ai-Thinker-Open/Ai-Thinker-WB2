...
void restore_key_init(uint8_t pin)
{
    retore.port = pin;
...
static void report_task(void *params)
{
    ...
    for (;;)
    {
        ...
        if (QCLOUD_RET_SUCCESS == deal_up_stream_user_logic(pReportDataList, &ReportCont)) {
            int rc = IOT_Template_JSON_ConstructReportArray(client, sg_data_report_buffer, sg_data_report_buffersize,
...
static void _refresh_local_property(void)
{
    ...
    for (int i = 0; i < TOTAL_PROPERTY_COUNT - DATA_TEMPLATE_WITHOUT_STRUCT; i++) {
        set_property_state(sg_DataTemplate[i].data_property.data, eCHANGED);
        // countdown_ms(reportTimer, 5000);
    }
}
