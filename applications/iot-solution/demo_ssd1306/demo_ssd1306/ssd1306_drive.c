...
/**
 * @brief Write Chinese characters in the corresponding position.
 * 
 * @param row Write on which row, must be greater than or equal to 1.
 * @param col Write in which column, must be greater than or equal to 1.
 * @param character_num Which Chinese character to use from the font library, with the index starting from 0.
 * @return int 
 */
int oled_output_char_chn(uint8_t row, uint8_t col, uint8_t character_num)
{
    row -= 1;
    col -= 1;
    //       buffer        row      column  line_break_offset
    memcpy(ssd1306_buff + 256*row + 16*col             ,     fonts_ch[character_num]     , 16); // The upper half
    memcpy(ssd1306_buff + 256*row + 16*col     +    128,     fonts_ch[character_num] + 16, 16); // The lower half
    return 0;
}

/**
 * @brief Write numbers in the corresponding position. Each position can hold two numbers, which can be placed either on the left or the right.
 * 
 * @param row Write on which row, must be greater than or equal to 1.
 * @param col Write in which column, must be greater than or equal to 1.
 * @param character_num Which number character to use from the font library, with the index starting from 0.
 * @param right 0: Place the character on the left, 1: Place the character on the right.
 * @return int 
 */
int oled_output_char_num(uint8_t row, uint8_t col, uint8_t character_num, uint8_t right)
{
    row -= 1;
    col -= 1;
    //       buffer        row      column  place_on_right line_break_offset
    memcpy(ssd1306_buff + 256*row + 16*col   +  8*right             ,      fonts_num[character_num]    , 8); // The upper half
    memcpy(ssd1306_buff + 256*row + 16*col   +  8*right     +    128,      fonts_num[character_num] + 8, 8); // The lower half
    return 0;
}


/**
 * @brief Display time function, can be called directly in a loop
 * 
 * @param yyyy year
 * @param MM month
 * @param dd day
...
int oled_time_output(uint16_t yyyy, uint8_t MM, uint8_t dd, uint8_t HH, uint8_t mm, uint8_t ss)
{
    ...
    // Character"时间" meaning of "time"
    oled_output_char_chn(1, 1, 0);
    oled_output_char_chn(1, 2, 1);

    // Character"年" meaning of "year"
    oled_output_char_num(2, 1, yyyy/1000, 0);
    oled_output_char_num(2, 1, yyyy/100%10, 1);
    oled_output_char_num(2, 2, yyyy%100/10, 0);
    oled_output_char_num(2, 2, yyyy%100%10, 1);
    oled_output_char_chn(2, 3, 2);

    // Character"月" meaning of "month"
    oled_output_char_num(2, 4, MM/10, 0);
    oled_output_char_num(2, 4, MM%10, 1);
    oled_output_char_chn(2, 5, 3);

    // Character"日" meaning of "day"
    oled_output_char_num(2, 6, dd/10, 0);
    oled_output_char_num(2, 6, dd%10, 1);
    oled_output_char_chn(2, 7, 4);


    // hour
    oled_output_char_num(3, 1, HH/10, 0);
    oled_output_char_num(3, 1, HH%10, 1);

    oled_output_char_num(3, 2, 10, 0); // :

    // minute
    oled_output_char_num(3, 2, mm/10, 1);
    oled_output_char_num(3, 3, mm%10, 0);

    oled_output_char_num(3, 3, 10, 1); // :

    // second
    oled_output_char_num(3, 4, ss/10, 0);
    oled_output_char_num(3, 4, ss%10, 1);
