...
static int32_t _subdev_operate_topic_map(subdev_handle_t *subdev_handle, aiot_mqtt_option_t option)
{
    ...
    for (idx = 0;idx < sizeof(g_subdev_topic_map)/sizeof(subdev_topic_map_t);idx++) {
        ...
        res = aiot_mqtt_setopt(subdev_handle->mqtt_handle, AIOT_MQTTOPT_APPEND_TOPIC_MAP, &map);
...
int32_t aiot_subdev_send_topo_add(void *handle, aiot_subdev_dev_t dev[], uint32_t dev_num)
{
    ...
    int32_t id = 0;
    char id_string[SUBDEV_ALINK_ID_MAX_LEN] = { 0 };
    subdev_handle_t *subdev_handle = (subdev_handle_t *)handle;
    ...
    /* alink id */
    memset(id_string, 0, SUBDEV_ALINK_ID_MAX_LEN);
    core_global_alink_id_next(subdev_handle->sysdep, &id);
    res = core_int2str(id, id_string, NULL);
    if (res < STATE_SUCCESS) {
        return res;
    }
    ...
    cJSON_AddStringToObject(root, "id", id_string);
