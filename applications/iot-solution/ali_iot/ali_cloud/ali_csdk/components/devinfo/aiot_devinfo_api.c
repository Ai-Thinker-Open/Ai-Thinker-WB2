...
static void _devinfo_mqtt_recv_handler(void *handle, const aiot_mqtt_recv_t *packet, void *userdata, aiot_devinfo_recv_type_t type)
{
    ...
    if (core_json_value((char *)packet->data.pub.payload, packet->data.pub.payload_len,
        ...
        if (core_str2uint(code_value, code_value_len, &code) == STATE_SUCCESS &&
            ...
            recv.type = type;
            if (AIOT_DEVINFORECV_GET_REPLY == type) {
                recv.data.get_reply.code = code;
                recv.data.get_reply.msg_id = id;
                recv.data.get_reply.data = data_value;
                recv.data.get_reply.data_len = data_value_len;
                recv.data.get_reply.message = message_value;
                recv.data.get_reply.message_len = message_value_len;
            } else {
...
static void _devinfo_generic_recv_handler(void *handle, const aiot_mqtt_recv_t *packet, void *userdata) {
   _devinfo_mqtt_recv_handler(handle, packet, userdata, AIOT_DEVINFORECV_GENERIC_REPLY);
}

static void _devinfo_get_recv_handler(void *handle, const aiot_mqtt_recv_t *packet, void *userdata) {
   _devinfo_mqtt_recv_handler(handle, packet, userdata, AIOT_DEVINFORECV_GET_REPLY);
}
...
static int32_t _devinfo_operate_topic_map(devinfo_handle_t *devinfo_handle, aiot_mqtt_option_t option)
{
    ...
    map.handler = _devinfo_generic_recv_handler;
...
static int32_t _devinfo_get_subscribe(void *mqtt_handle, devinfo_handle_t* devinfo_handle)
{
    ...
    if(devinfo_handle == NULL || mqtt_handle ==NULL) {
        return STATE_USER_INPUT_NULL_POINTER;
    }
    sysdep = devinfo_handle->sysdep;

    res = core_sprintf(sysdep, &topic, DEVINFO_GET_REPLY_TOPIC_FMT, topic_src, sizeof(topic_src) / sizeof(char *), DEVINFO_MODULE_NAME);
    if (res != STATE_SUCCESS) {
        return res;
    }

    res = aiot_mqtt_sub(mqtt_handle, topic, _devinfo_get_recv_handler, 0, (void *)devinfo_handle);
...
int32_t aiot_devinfo_setopt(void *handle, aiot_devinfo_option_t option, void *data)
{
    ...
    switch (option) {
        case AIOT_DEVINFOOPT_MQTT_HANDLE: {
            ...
            if (res >= STATE_SUCCESS) {
                res = _devinfo_get_subscribe(devinfo_handle->mqtt_handle, devinfo_handle);
            }
            devinfo_handle->sysdep->core_sysdep_mutex_lock(devinfo_handle->data_mutex);
        }
        break;
...
int32_t aiot_devinfo_send(void *handle, aiot_devinfo_msg_t *msg)
{
    ...
    switch (msg->type) {
        ...
        break;
        case AIOT_DEVINFO_MSG_GET: {
            if (NULL == msg->data.get.params) {
                res = STATE_USER_INPUT_NULL_POINTER;
            } else {
                res = _devinfo_send(devinfo_handle, msg->product_key, msg->device_name, &msg->data.get, DEVINFO_GET_TOPIC_FMT);
            }
        }
        break;
        default: {
