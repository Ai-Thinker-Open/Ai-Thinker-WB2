...
static int st7796s_drive_clear(uint16_t color)
{
    ...
    blog_info("st7796 width:%d heiht:%d", width, height);
    for (i = 0;i<height;i++)
    {
        for (m = 0;m<width;m++)
        {
...
int st7796s_drive_init(void)
{
    ...
    st7796_drive_set_orientation(LV_DISPLAY_LANDSCAPE);
...
int st7796_drive_set_pixels(uint16_t x, uint16_t y, uint16_t color)
{
    ...
    st7796s_set_windows(x, x+10, y, y+10);
