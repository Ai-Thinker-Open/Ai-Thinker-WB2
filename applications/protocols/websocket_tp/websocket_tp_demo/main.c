#include <FreeRTOS.h>
#include <task.h>
#include <stdio.h>
#include <string.h>

#include <blog.h>
#include <aos/yloop.h>
#include <aos/kernel.h>
#include <lwip/tcpip.h>
#include <wifi_mgmr_ext.h>
#include <hal_wifi.h>

#include "websocket_tp_bridge.h"

#define ROUTER_SSID "HUAWEI"
#define ROUTER_PWD "fae12345678"

static wifi_conf_t conf = {
    .country_code = "CN",
};

static void wifi_sta_connect(char *ssid, char *password)
{
    wifi_interface_t wifi_interface = wifi_mgmr_sta_enable();
    wifi_mgmr_sta_connect(wifi_interface, ssid, password, NULL, NULL, 0, 0);
}

static void event_cb_wifi_event(input_event_t *event, void *private_data)
{
    (void)private_data;

    switch (event->code) {
    case CODE_WIFI_ON_INIT_DONE:
        blog_info("[APP] [EVT] INIT DONE %lld", aos_now_ms());
        wifi_mgmr_start_background(&conf);
        break;
    case CODE_WIFI_ON_MGMR_DONE:
        blog_info("[APP] [EVT] MGMR DONE %lld", aos_now_ms());
        wifi_sta_connect(ROUTER_SSID, ROUTER_PWD);
        break;
    case CODE_WIFI_ON_SCAN_DONE:
        blog_info("[APP] [EVT] SCAN Done %lld", aos_now_ms());
        break;
    case CODE_WIFI_ON_DISCONNECT:
        blog_info("[APP] [EVT] disconnect %lld", aos_now_ms());
        break;
    case CODE_WIFI_ON_CONNECTING:
        blog_info("[APP] [EVT] Connecting %lld", aos_now_ms());
        break;
    case CODE_WIFI_CMD_RECONNECT:
        blog_info("[APP] [EVT] Reconnect %lld", aos_now_ms());
        break;
    case CODE_WIFI_ON_CONNECTED:
        blog_info("[APP] [EVT] connected %lld", aos_now_ms());
        break;
    case CODE_WIFI_ON_PRE_GOT_IP:
        blog_info("[APP] [EVT] PRE GOT IP %lld", aos_now_ms());
        break;
    case CODE_WIFI_ON_GOT_IP:
        blog_info("[APP] [EVT] GOT IP %lld", aos_now_ms());
        blog_info("[SYS] heap free ~%u bytes", (unsigned)xPortGetFreeHeapSize());
        websocket_tp_bridge_start();
        break;
    case CODE_WIFI_ON_PROV_BSSID:
        blog_info("[APP] [EVT] [PROV] [BSSID] %lld: %s", aos_now_ms(),
                  event->value ? (const char *)event->value : "UNKNOWN");
        if (event->value) {
            vPortFree((void *)event->value);
        }
        break;
    case CODE_WIFI_ON_PROV_DISCONNECT:
        blog_info("[APP] [EVT] [PROV] [DISCONNECT] %lld", aos_now_ms());
        break;
    default:
        blog_info("[APP] [EVT] code %u, %lld", event->code, aos_now_ms());
        break;
    }
}

static void proc_main_entry(void *pvParameters)
{
    (void)pvParameters;
    aos_register_event_filter(EV_WIFI, event_cb_wifi_event, NULL);
    hal_wifi_start_firmware_task();
    aos_post_event(EV_WIFI, CODE_WIFI_ON_INIT_DONE, 0);
    vTaskDelete(NULL);
}

void main(void)
{
    blog_set_level_log_component(BLOG_LEVEL_INFO, "ws_tp");
    blog_set_level_log_component(BLOG_LEVEL_WARN, "axk_tls");
    blog_set_level_log_component(BLOG_LEVEL_WARN, "tcp_transport");

    puts("[OS] tcpip_init...");
    tcpip_init(NULL, NULL);
    xTaskCreate(proc_main_entry, "main_entry", 1024, NULL, 15, NULL);
}
