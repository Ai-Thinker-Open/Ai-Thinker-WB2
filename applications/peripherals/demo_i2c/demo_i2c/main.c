...
int main(void) {
  ...
  for (;;) {
    ...
    hosal_i2c_master_send(&i2c0, SHT31_DEFAULT_ADDR, command, sizeof command,
                          100);
    hosal_i2c_master_recv(&i2c0, SHT31_DEFAULT_ADDR, (uint8_t *)&data,
                          sizeof data, 100);
    ...
    if (crc8(&data.st_high, 2) == data.st_crc8) {
      ...
      sprintf(temperature_str, "%d.%02u C", temperature_integer,
              temperature_decimal);
    } else {
