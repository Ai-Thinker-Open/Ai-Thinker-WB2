...
static void dns_server_send_reply(struct dns_server_ctx *dns_ctx)
{
    ip4_addr_t dns_ip;
    struct pbuf *rp = NULL;
    ...
    qry.addr   = dns_ip.addr;

    pbuf_take_at(rp, &qry, SIZEOF_DNSANS_HDRQUE, query_idx);
    pbuf_realloc(rp, query_idx + SIZEOF_DNSANS_HDRQUE);

    udp_sendto(dns_ctx->upcb1, rp, dns_ctx->addr1, dns_ctx->port1);
    pbuf_free(rp);
}

static void dns_request_handler(void *arg, struct udp_pcb *upcb, struct pbuf *p, const ip_addr_t *addr, u16_t port)
{
    struct dns_ans_hdr hdr;
    ...
    pbuf_copy_partial(
        p,
        dns_ctx.name,
        len = p->tot_len > (int)sizeof(dns_ctx.name) - 1 ? (int)sizeof(dns_ctx.name) - 1 : p->tot_len,
        SIZEOF_DNSANS_HDR
    );
    ...
    printf("[captive] name: %s\r\n", dns_ctx.name);

    pbuf_free(p);
    dns_server_send_reply(&dns_ctx);
}
...
void captive_portal_start(void)
{
    ...
    if (s_captive_pcb != NULL) {
        return;
    }

    s_captive_pcb = udp_new();
    if (!s_captive_pcb) {
        printf("captive: udp_new failed\r\n");
        return;
    }

    err = udp_bind(s_captive_pcb, IP_ADDR_ANY, 53);
    if (err != ERR_OK) {
        printf("captive: bind 53 failed %d (DNS may be served by sdk dns_server)\r\n", (int)err);
        udp_remove(s_captive_pcb);
        s_captive_pcb = NULL;
        return;
    }

    udp_recv(s_captive_pcb, dns_request_handler, NULL);
    web_popup_dns_ready();
}
