...
void mbedtls_hamcSHA256_test(void)
{
    uint8_t* outbuff = pvPortMalloc(32);

    mbedtls_md_context_t hmacsha1_ctx;
    mbedtls_md_info_t* md_info = mbedtls_md_info_from_type(MBEDTLS_MD_SHA256);
    ...
    blog_info_hexdump("Segmental encryption", outbuff, 32);
    ...
    blog_info_hexdump("Overall encryption", outbuff, 32);
