...
void mbedtls_hamcSHA512_test(void)
{
    uint8_t* outbuff = pvPortMalloc(64);

    mbedtls_md_context_t hmacsha1_ctx;
    mbedtls_md_info_t* md_info = mbedtls_md_info_from_type(MBEDTLS_MD_SHA512);
    ...
    blog_info_hexdump("Segmental encryption", outbuff, 64);
    ...
    blog_info_hexdump("Overall encryption", outbuff, 64);
